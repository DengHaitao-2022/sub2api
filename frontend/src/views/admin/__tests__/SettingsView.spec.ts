...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("submits gateway audit settings", async () => {
    getSettings.mockResolvedValueOnce({
      ...
      gateway_audit_input_message_policy: "last_user_message",
      gateway_audit_index_enabled: true,
  ...
  it("preserves audit settings when a legacy save response clears the OAuth rate", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      ...
      gateway_audit_input_capture_mode: "full",
      gateway_audit_output_capture_mode: "hash",
      gateway_audit_max_input_body_bytes: 131072,
    });
    updateSettings.mockResolvedValueOnce({
      ...
      gateway_audit_enabled: false,
      gateway_audit_input_capture_mode: "",
      gateway_audit_output_capture_mode: "",
      gateway_audit_max_input_body_bytes: 0,
    });
    const wrapper = mountView();
    await flushPromises();
    await wrapper.find("form").trigger("submit.prevent");
    await flushPromises();
    expect(wrapper.get<HTMLInputElement>('[data-testid="openai-oauth-scheduling-rate-multiplier"]').element.value).toBe("");
    await wrapper.find("form").trigger("submit.prevent");
    await flushPromises();
